...
The current release is **0.5.3 beta**. Update `VERSION`, the editor footer, bundle build number, changelog, badges/download links and release notes together. Do not claim complete legacy parity or notarization unless the relevant acceptance gate passes.
...
The 0.5.3 release has **no Developer ID identity and no Apple notarization ticket**. It must be labeled as a development prerelease with explicit first-launch instructions.
